...
class SimpleMessageQueue:
    ...
    def _should_deduplicate(self, wrapper: MessageWrapper) -> bool:
        ...
        shop_id = str(getattr(kwargs, "shop_id", "") or "").strip()
        user_id = str(getattr(kwargs, "user_id", "") or "").strip()
        from_uid = str(getattr(kwargs, "from_uid", "") or "").strip()
        ...
        dedup_source = "|".join(
            ...
            for part in (
                ...
                user_id,
                from_uid,
                msg_id or wrapper.message_id,
                context_type,
                content_str,
            )
            if part
        )
        content_hash = hashlib.md5(dedup_source.encode("utf-8")).hexdigest()
