...
class MessageConsumer:
    ...
    def _extract_conversation_key(self, context: Context) -> str:
        ...
        try:
            ...
            shop_id = str(getattr(kwargs, 'shop_id', None) or "unknown_shop")
            user_id = str(getattr(kwargs, 'user_id', None) or "unknown_user")
            from_uid = str(getattr(kwargs, 'from_uid', None) or "unknown_customer")
            return f"{channel_str}:{shop_id}:{user_id}:{from_uid}"
        except Exception as e:
            self.logger.error(f"Failed to extract conversation key: {e}")
