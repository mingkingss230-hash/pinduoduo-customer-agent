...
class LifecycleMixin:
    ...
    def _build_queue_name(shop_id: str, user_id: str) -> str:
        ...
        return f"pdd_{shop_id}_{user_id}"
    ...
    async def init(self, shop_id: str, user_id: str, username: str, on_success: callable, on_failure: callable):
        ...
        try:
                ...
                try:
                    ...
                    if heartbeat_task:
                        try:
                            ...
                            pass
                    await self._cleanup_resources(self._build_queue_name(shop_id, user_id))
            ...
            await self._cleanup_resources(self._build_queue_name(shop_id, user_id))
            if self.reconnect_config.enable_auto_reconnect and not (self._stop_event and self._stop_event.is_set()):
                raise RuntimeError(f"WebSocket连接已关闭: {e}")
            ...
            await self._cleanup_resources(self._build_queue_name(shop_id, user_id))
            if self.reconnect_config.enable_auto_reconnect and not (self._stop_event and self._stop_event.is_set()):
                raise RuntimeError(f"WebSocket连接错误: {e}")
